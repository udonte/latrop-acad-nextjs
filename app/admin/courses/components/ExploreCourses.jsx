...
const ExploreCourses = () => {
	...
	const getFilteredCourses = () => {
		...
		if (activeTab === TABS[1]) return filterCourses(ENGINEERING);
		if (activeTab === TABS[2]) return filterCourses(PRODUCT);
		if (activeTab === TABS[3]) return filterCourses(DATA);
		if (activeTab === TABS[4]) return filterCourses(BUSINESS);
	};
			...
			<div className="relative flex flex-col gap-7 px-[3%] pt-5 pb-20">
				<div className="flex gap-2 md:justify-between">
					...
					>
						...
						>
							<p className="text-[#222222]">{activeTab}</p>
							<svg
					...
					<div className="hidden md:flex items-center border-b border-[#2222221A]">
						{TABS.map((tab, index) => (
							<p
								key={index}
								onClick={() => changeTab(tab)}
								className={`py-2.5 px-3 md:px-3 cursor-pointer text-[14px] md:text-[15px] ${
									activeTab === tab
					...
					<div className="hidden md:flex bg-[#ffffff] items-center w-[192px] lg:w-[300px] py-2 px-3 lg:px-4 border border-[#2222221A] rounded-lg">
						<div className="flex gap-2 lg:gap-4 items-center grow">
							...
							<input
								...
								className="rounded-lg h-[17px] grow placeholder:text-[#AAAAAA] text-[13px] placeholder:text-[13px] focus:outline-none"
							/>
						</div>
						{/* <Image
							src={SEARCH.filter}
				...
				{showSearch && (
					<div className="md:hidden flex bg-[#2222220D] items-center w-full py-2 px-3 lg:px-4 border border-[#22222226] rounded-lg">
						<div className="flex gap-2 lg:gap-4 items-center grow">
							<Image
								src={SEARCH.searchB}
								width={1000}
							...
							<input
								...
								value={searchTerm}
								onChange={(e) => setSearchTerm(e.target.value)}
								className="rounded-lg h-[17px] bg-transparent grow placeholder:text-[#AAAAAA] text-[13px] placeholder:text-[13px] focus:outline-none"
							/>
						</div>
						<Image
							src={SEARCH.close}
				...
				<Courses course={getFilteredCourses()} />
			</div>
		</div>
	);
};
