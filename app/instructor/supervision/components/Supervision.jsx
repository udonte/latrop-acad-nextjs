...
const Supervision = () => {
	const [activeTab, setActiveTab] = useState("Student Tasks");
	...
	const renderTabContent = () => {
		switch (activeTab) {
			...
			case "Student Attendance":
				return <Attendance searchTerm={searchTerm} />;
					...
					<div className="hidden md:flex items-center border-b border-[#2222221A]">
						{TABS.map((tab, index) => (
							<p
								key={index}
								onClick={() => changeTab(tab)}
								className={`py-2.5 px-3 md:px-3 cursor-pointer text-[14px] lg:text-[13px] ${
									activeTab === tab
