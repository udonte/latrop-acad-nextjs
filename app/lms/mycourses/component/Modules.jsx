	...
	return (
		<div className="py-2 px-2 md:py-6 md:px-4 lg:py-10 lg:px-10 bg-white rounded-lg border border-[#A020F01A] flex flex-col gap-">
			{COURSEDETAILS.map((module, index) => (
				<div key={index} className="flex flex-col gap-[14px] mb-4 border-b border-[#2222221A]">
						...
						{module.status === "Completed" && (
							<div className="hidden md:flex gap-1 items-center">
								<Image
									...
									className="w-5 h-5"
								/>
								<p className="text-[#A020F0] text-[15px] font-medium">
									{module.status}
								</p>
							</div>
						)}
						{module.status === "Inprogress" && (
							<button className="hidden md:flex items-center border-[1.5px] buttons bg-[#A020F0] w-max h-min rounded-[5px] py-4 px-6 text-[#ffffff] text-[15px] leading-[20px] font-medium">
								{INPROGRESSBUTTON}
							</button>
						)}
						{module.status === "Locked" && (
							<div className="hidden md:flex items-center justify-center w-[113px]">
								<Image
									width={1000}
									height={1000}
									src={LOCKED}
